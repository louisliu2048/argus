...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=argus \
          -X github.com/cosmos/cosmos-sdk/version.AppName=$(ARGUS_BINARY) \
...
build-docker:
	...
	$(DOCKER) rm argus || true
	# create a new container from the latest image
	$(DOCKER) create --name argus -t -i ${DOCKER_IMAGE}:latest argus
	# move the binaries to the ./build directory
	mkdir -p ./build/
	$(DOCKER) cp argus:/usr/bin/argusd ./build/
...
godocs:
	@echo "--> Wait a few seconds and visit http://localhost:6060/pkg/github.com/louisliu2048/argus/types"
	godoc -http=:6060
...
test-sim-custom-genesis-fast:
	...
	@echo "By default, ${HOME}/.$(ARGUS_DIR)/config/genesis.json will be used."
	@go test -mod=readonly $(SIMAPP) -run TestFullAppSimulation -Genesis=${HOME}/.$(ARGUS_DIR)/config/genesis.json \
...
test-sim-custom-genesis-multi-seed: runsim
	...
	@echo "By default, ${HOME}/.$(ARGUS_DIR)/config/genesis.json will be used."
	@$(BINDIR)/runsim -Genesis=${HOME}/.$(ARGUS_DIR)/config/genesis.json -SimAppPkg=$(SIMAPP) -ExitOnFail 400 5 TestFullAppSimulation
...
format:
	...
	find . -name '*.go' -type f -not -path "./vendor*" -not -path "*.git*" -not -path "./client/docs/statik/statik.go" -not -name '*.pb.go' | xargs goimports -w -local github.com/louisliu2048/argus
.PHONY: format
...
localnet-start: localnet-stop localnet-build
	@if ! [ -f build/node0/$(ARGUS_BINARY)/config/genesis.json ]; then docker run --rm -v $(CURDIR)/build:/argus:Z argus/node "./argusd testnet init-files --v 4 -o /argus --keyring-backend=test --starting-ip-address 192.167.10.2"; fi
	docker-compose up -d
...
localnet-unsafe-reset:
	docker-compose down
ifeq ($(OS),Windows_NT)
	@docker run --rm -v $(CURDIR)\build\node0\argusd:/argus\Z argus/node "./argusd tendermint unsafe-reset-all --home=/argus"
	@docker run --rm -v $(CURDIR)\build\node1\argusd:/argus\Z argus/node "./argusd tendermint unsafe-reset-all --home=/argus"
	@docker run --rm -v $(CURDIR)\build\node2\argusd:/argus\Z argus/node "./argusd tendermint unsafe-reset-all --home=/argus"
	@docker run --rm -v $(CURDIR)\build\node3\argusd:/argus\Z argus/node "./argusd tendermint unsafe-reset-all --home=/argus"
else
	@docker run --rm -v $(CURDIR)/build/node0/argusd:/argus:Z argus/node "./argusd tendermint unsafe-reset-all --home=/argus"
	@docker run --rm -v $(CURDIR)/build/node1/argusd:/argus:Z argus/node "./argusd tendermint unsafe-reset-all --home=/argus"
	@docker run --rm -v $(CURDIR)/build/node2/argusd:/argus:Z argus/node "./argusd tendermint unsafe-reset-all --home=/argus"
	@docker run --rm -v $(CURDIR)/build/node3/argusd:/argus:Z argus/node "./argusd tendermint unsafe-reset-all --home=/argus"
endif
...
PACKAGE_NAME:=github.com/louisliu2048/argus
