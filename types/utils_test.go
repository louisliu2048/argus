...
func init() {
	cfg := sdk.GetConfig()
	cfg.SetBech32PrefixForAccount("argus", "arguspub")
}
...
func TestGetArgusAddressFromBech32(t *testing.T) {
	testCases := []struct {
		name       string
		address    string
		expAddress string
		...
		{
			"argus address",
			"argus1qql8ag4cluz6r4dz28p3w00dnc9w8ueuafmxps",
			"argus1qql8ag4cluz6r4dz28p3w00dnc9w8ueuafmxps",
			false,
		},
		{
	...
	for _, tc := range testCases {
		addr, err := GetArgusAddressFromBech32(tc.address)
		if tc.expError {
