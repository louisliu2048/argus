...
const (
	// MainnetChainID defines the Argus EIP155 chain ID for mainnet
	MainnetChainID = "argus_9001"
	// TestnetChainID defines the Argus EIP155 chain ID for testnet
	TestnetChainID = "argus_9000"
)

// IsMainnet returns true if the chain-id has the Argus mainnet EIP155 chain prefix.
func IsMainnet(chainID string) bool {
	return strings.HasPrefix(chainID, MainnetChainID)
}

// IsTestnet returns true if the chain-id has the Argus testnet EIP155 chain prefix.
...
// GetArgusAddressFromBech32 returns the sdk.Account address of given address,
// while also changing bech32 human readable prefix (HRP) to the value set on
// the global sdk.Config (eg: `argus`).
// The function fails if the provided bech32 address is invalid.
func GetArgusAddressFromBech32(address string) (sdk.AccAddress, error) {
