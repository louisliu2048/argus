...
Before upgrading the Evmos version. Stop your instance of `argusd` using `Ctrl/Cmd+C`.

Next, upgrade the software to the desired release version. Check the Evmos [releases page](https://github.com/louisliu2048/argus/releases) for details on each release.
...
::: tip
If the software version does not match, then please check your `$PATH` to ensure the correct `argusd` is running.
:::
...
```bash
rm $HOME/.argusd/config/addrbook.json
