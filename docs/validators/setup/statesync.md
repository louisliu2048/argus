...
```bash
argusd init $moniker --chain-id $CHAIN_ID
```

Move genesis file to .argusd/config folder

```bash
mv $HOME/genesis.json ~/.argusd/config/
```
...
```bash
sed -i -e "s%^moniker *=.*%moniker = \"$moniker\"%; " $HOME/.argusd/config/config.toml
sed -i -e "s%^indexer *=.*%indexer = \"null\"%; " $HOME/.argusd/config/config.toml
sed -i -e "s%^persistent_peers *=.*%persistent_peers = \"$PEER\"%; " $HOME/.argusd/config/config.toml
```
...
s|^(seeds[[:space:]]+=[[:space:]]+).*$|\1\"\"|" ~/.argusd/config/config.toml
```

### Create argusd service

```bash
echo "[Unit]
Description=argusd Node
After=network.target
#
[Service]
User=$USER
Type=simple
ExecStart=$(which argusd) start
Restart=on-failure
LimitNOFILE=65535
#
[Install]
WantedBy=multi-user.target" > $HOME/argusd.service; sudo mv $HOME/argusd.service /etc/systemd/system/
```

```bash
sudo systemctl enable argusd.service && sudo systemctl daemon-reload
```

### Run argusd

```bash
sytemctl start argusd
```

### Check logs

```bash
journalctl -u argusd -f
```
...
```bash
sed -i.bak -E "s|^(enable[[:space:]]+=[[:space:]]+).*$|\1false|" $HOME/.argusd/config/config.toml
```
