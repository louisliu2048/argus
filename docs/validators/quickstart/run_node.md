...
```bash
argusd start --json-rpc.enable=true --json-rpc.api="eth,web3,net"
```

## Key Management

To run a node with the same key every time: replace `argusd keys add $KEY` in `./init.sh` with:

```bash
echo "your mnemonic here" | argusd keys add $KEY --recover
```
...
```bash
argusd keys unsafe-export-eth-key $KEY
```
...
```bash
argusd config keyring-backend file
```
...
```bash
argusd start --evm.tracer $TRACER --tracestore $TRACESTORE --pruning=nothing $TRACE --log_level $LOGLEVEL --minimum-gas-prices=0.0001aevmos --json-rpc.api eth,txpool,personal,net,debug,web3
```
...
::: danger
If you are running a **validator node**, always be careful when doing `argusd unsafe-reset-all`. You should never use this command if you are not switching `chain-id`.
:::
...
```shell
rate(argusd_transactions_processed[1m])
```
