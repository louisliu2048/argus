...
This command creates a 4-node network using the `argusdnode` Docker image.
...
Creating argusdnode0 ... done
Creating argusdnode2 ... done
Creating argusdnode1 ... done
Creating argusdnode3 ... done
```
...
calling the `argusd testnet` command. This outputs a handful of files in the
`./build` directory:
...
Each `./build/nodeN` directory is mounted to the `/argusd` directory in each container.
...
```bash
# node 0: daemon logs
docker exec argusdnode0 tail argusd.log
...
To interact with `argusd` and start querying state or creating txs, you use the
`argusd` directory of any given node as your `home`, for example:

```bash
argusd keys list --home ./build/node0/argusd
```
...
::: tip
**Note**: Each node's seed is located at `./build/nodeN/argusd/key_seed.json` and can be restored to the CLI using the `argusd keys add --restore` command
:::
