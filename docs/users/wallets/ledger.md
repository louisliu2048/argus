...
2. **I can’t generate keys using the CLI with `argusd` with the `--ledger` flag**

CLI bindings with `argusd` binary are not currently supported. In the meantime, you can use the Ethereum Ledger App with EIP712 using [evmos.me](https://evmos.me). See the [`EIP712 Signing`](#eip712-signing) section for reference.

3. **I can’t generate a key for the Evmos native multisig using the `argusd` CLI and and Ledger**

You can generate a multisig wallet using the `argusd` CLI, although the `--ledger` option is not available at the moment.
