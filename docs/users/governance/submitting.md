...
Users can query the proposal details with the `argusd` command-line interface using this command:

```bash
`argusd --node https://tendermint.bd.evmos.org:26657 query gov proposal 23`.
```
...
```bash
`argusd --node https://tendermint.bd.evmos.org:26657 query gov proposal 7`.
```
...
For information on how to use `argusd` binary to submit an on-chain proposal through the governance module, please refer to the [quickstart](../../validators/quickstart/binary.md) documentation.

### CLI

This is the command format for using `argusd` (the command-line interface) to submit your proposal on-chain:
...
1. `argusd` is the command-line interface client that is used to send transactions and query Evmos
...
6. `--chain-id evmos_90001-2` is Evmos Mainnet. For current and past chain-id's, please look at the [Chain ID](./../technical_concepts/chain_id.md) documentation.
   - the testnet chain ID is [evmos_9000-4](https://testnet.mintscan.io/evmos). For current and past testnet information, please look at the [testnet repository](https://github.com/louisliu2048/testnets)
7. `--node` is using a full node to send the transaction to the Evmos Mainnet

### Verifying your transaction

After posting your transaction, your command line interface (`argusd`) will provide you with the transaction's hash, which you can either query using `argusd` or by searching the transaction hash using [Mintscan](https://www.mintscan.io/evmos) or any block explorer.
...
```bash
argusd q gov proposals --depositor evmos1hxv7mpztvln45eghez6evw2ypcw4vjmsmr8cdx
```

Once you have the proposal ID, this is the command to deposit extra tokens:

```bash
argusd tx gov deposit <proposal-id> <deposit> --from <name>
```
